...
const Hero = () => {
  ...
  // Refs pour mesurer les éléments
  ...
  // Calcul la hauteur du hero et définit le mode plein écran
  useEffect(() => {
  ...
  // Effet pour ajuster dynamiquement la taille du logo
  useEffect(() => {
    if (!isFullHeight || !heroHeight) return;

    const adjustLogoSize = () => {
      if (!heroRef.current || !titleRef.current || !cardsRef.current || !ctaRef.current) return;
      
      const totalHeight = heroHeight;
      const titleHeight = titleRef.current.offsetHeight;
      const cardsHeight = cardsRef.current.offsetHeight;
      const ctaHeight = ctaRef.current.offsetHeight;
      
      // Calcul des marges approximatives
      const topMargin = 20; // ~lg:mt-6
      const titleBottomMargin = 0; // inclus dans pb-12
      const cardsTopMargin = 0; // inclus dans le container
      const ctaTopMargin = 48; // ~mt-12
      const ctaBottomMargin = 20; // marge en bas
      
      // Hauteur totale nécessaire pour les éléments en dehors du logo
      const requiredHeight = titleHeight + titleBottomMargin + 
                            cardsTopMargin + cardsHeight + 
                            ctaTopMargin + ctaHeight + ctaBottomMargin;
      
      // Espace disponible pour le logo
      const availableLogoSpace = totalHeight - requiredHeight - topMargin - ctaBottomMargin;
      
      // Limiter la taille du logo entre 80px et 300px
      const newLogoHeight = Math.max(80, Math.min(400, availableLogoSpace));
      
      setLogoHeight(newLogoHeight);
    };
    
    // Ajuster après un court délai pour laisser le temps au DOM de se mettre à jour
    const timer = setTimeout(adjustLogoSize, 100);
    ...
    >
      <div className="container-kote relative z-10 flex flex-col h-full lg:justify-start">
        ...
        <div 
          ...
          className={`flex justify-center items-center mt-2 sm:mt-3 md:mt-4 lg:mt-6 mb-2 sm:mb-3 md:mb-4 lg:mb-2 relative z-[2000]`}
        >
          <div 
            className={`relative max-w-[400px] sm:max-w-[400px] md:max-w-[400px] lg:max-w-[400px] xl:max-w-[500px] transform transition-all duration-700 md:w-auto`}
            style={{ height: isFullHeight ? `${logoHeight}px` : '200px' }}
          >
            ...
            <div className="relative h-full w-full overflow-hidden">
              <img 
                ...
                className="w-auto mx-auto min-w-[180px] object-contain object-top relative z-10 drop-shadow-xl"
